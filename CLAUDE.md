...
- **Scaffolder rules**: `*.types.ts`, `types.ts`, `remote.ts`, `routes.d.ts` are regenerated every dev run ("do not edit" headers); `.svelte` components, `mount.ts`, `render.ts`, `vite.config.ts`, `package.json` are write-once and user-owned after creation. Each sample commits `wwwroot/.gitkeep` — Development crashes without the directory.
...
- Generated `remote.ts` in `samples/RemoteFunctions` is committed; regenerate it by running the sample in Development after changing `TodoApi` or the generator output shape.
