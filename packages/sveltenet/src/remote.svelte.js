...
// Use through generated per-service `*.remote.ts` classes — they wire paths and types.
...
const queryFinalizer =
	typeof FinalizationRegistry === 'function'
		? new FinalizationRegistry(({ reference, cache, key }) => {
				activeQueries.delete(reference);
				if (cache.get(key) === reference) cache.delete(key);
			})
		: undefined;

function retainQuery(instance, cache, key) {
	...
	activeQueries.add(reference);
	queryFinalizer?.register(instance, { reference, cache, key }, reference);
}

function dereferenceQuery(reference) {
...
export function query(path, mapArgs) {
	...
	return (...args) => {
		...
		// Per-render dedup is hydratable's job — same key, same stashed promise.
			...
			let instance = dereferenceQuery(cache.get(key));
			if (!instance) {
				instance = new RemoteQuery(path, argsObject);
				retainQuery(instance, cache, key);
			}
			return instance;
	};
}
...
export function refreshAll() {
	const refreshes = [];
	for (const reference of activeQueries) {
		const instance = dereferenceQuery(reference);
		if (instance) refreshes.push(instance.refresh().catch(() => undefined));
		else activeQueries.delete(reference);
	}
